...
META_CACHE = (
    (OUTD / "meta_height_30m.npy")
    if (OUTD / "meta_height_30m.npy").exists()
    else (_TMP_LAB / "meta_height_30m.npy")
)
...
def build_features(yr):
    ...
    with rasterio.open(COMP / f"s2_{yr}.tif") as s:
        ...
        nir = s.read(2).astype(np.float32)
    with rasterio.open(COMP / f"s2_rgb_{yr}.tif") as s:
        green = s.read(2).astype(np.float32)
        blue = s.read(3).astype(np.float32)
    den = nir + red
    ...
    feat = np.stack(
        [np.nan_to_num(ndvi), dw, META_H, ESATREE, red, nir, green, blue, gndvi, nir_red], axis=-1
    ).reshape(-1, len(FEATS))
...
def main():
    ...
    json.dump(
        ...
        open(OUTD / "canopy_clf_meta.json", "w"),
        indent=2,
    )
    ...
    for yr in YEARS:
        if not (COMP / f"s2_{yr}.tif").exists():
            print(f"[model-canopy] {yr}: missing s2_{yr}.tif; skip")
            continue
        feat, ndvi, valid = build_features(yr) if yr != 2021 else (feat2021, ndvi2021, valid2021)
        proba = clf.predict_proba(feat)[:, 1].reshape(H, W)
        canopy = (proba >= best_thr).astype(np.uint8)
        canopy[~valid] = 255
        base = (ndvi >= 0.62).astype(np.uint8)
        base[~valid] = 255
        bpath = COMP / f"canopy_{yr}.tif"
        ndvi_backup = COMP / f"canopy_ndvi_{yr}.tif"
    ...
    with open(OUTD / "ncr_series_model_vs_ndvi.csv", "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["year", "model_pct", "ndvi_pct"])
        w.writeheader()
        w.writerows(rows)
